...
export class Conllu implements IParser {
    ...
    getTokenCurrentSentence(tokenIndex: number): ConllToken {
        return this.sentence.value[tokenIndex];
    }

    /**
     * Updates a token in the current sentence given an index.
    ...
    updateTokenCurrentSentence(tokenIndex: number, token: ConllToken): void {
        ...
        conllToken.lemma = token.lemma;
        conllToken.upos = token.upos;
        conllToken.xpos = token.xpos;
        conllToken.feat = token.feat;
        conllToken.head = token.head;
        conllToken.deprel = token.deprel;
        conllToken.deps = token.deps;
        conllToken.misc = token.misc;
    }

    /**
     * Returns the length of the current sentence
     */
    currentSentenceLength(): number {
        return this.sentence.value.length;
    }
}
