...
export class StoreService {
  ...
  annotation: string = 'Ner';
  index = 0;
  store: object;
  nbSentences: number;

  constructor() {
    this.store = { 'Conll-U': conlluSents, Ner: nerSents };
    this.nbSentences = conlluSents.length;

    console.log(this.store);
    this.sentence$.next(this.store[this.annotation][0]);
  }
   ...
   * @param toRight next or previous sentence
   */
  nextSentence(toRight: boolean) {
    if (toRight) {
